...
# PyPSA-DE scenarios are generated by ``build_scenarios`` in
# rules/pypsa-de/modifications.smk. Do not expose PyPSA-Eur's generic
# ``create_scenarios`` rule here: both rules would claim the configured
# run.scenarios.file, while only build_scenarios adds the Ariadne/IIASA data
# required by the German model.
