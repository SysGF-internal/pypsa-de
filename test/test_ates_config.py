...
def ates_config(*, data_source, data_file=None, heat_sources=None):
    return {
        "sector": {
            "district_heating": {
                "ptes": {"enable": False},
...
def test_hi_grid_ates_requires_private_input_and_heat_source():
    with pytest.raises(ValidationError, match="'ates' is not in"):
        validate_config(ates_config(data_source="hi_grid"))
...
def test_hi_grid_ates_accepts_explicit_input():
